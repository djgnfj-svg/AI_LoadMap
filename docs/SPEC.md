...
**세 층 모두 형식이 있다.** 전까지는 티켓만 있었고, 그 위 두 층은 제목 한 줄이 전부였다. 그러면 주가 끝났는지를 티켓 개수로만 판단하게 된다 — 티켓을 다 닫았는데 그 주의 목표는 안 선 경우를 잡을 수 없다.

층마다 맡는 일이 달라 형식도 다르다. 제목이 「무엇이 있는가」면 본문은 「그걸 어떻게 확인하는가」다. 제목에 쓴 것을 본문에서 되풀이하지 않는다.

**주** — 관리 단위다. 범위를 좁히는 자리가 필요하다.

```markdown
## 확인
- [ ] 적 3종이 플레이어를 추격한다
- [ ] 피격 시 체력이 깎이고 0에서 죽는다

## 안 하는 것
- 보스 · 밸런싱 (7주차)
```

**태스크** — 한 덩어리다. 한 줄로 끝난다. 「참고」는 티켓에만 있다.

```markdown
## 무엇을
플레이어 입력에서 피격 판정까지 한 바퀴.
...
주가 2개인 것은 관리 단위라 여러 면을 봐야 해서고, 태스크가 1개인 것은 한 덩어리라서다. **범위를 좁히는 절(`안 하는 것`)은 주에만 둔다** — 태스크·티켓은 이미 범위가 좁다.

검사는 세 층이 같은 파서를 쓴다 (`graphs/critic.py`). 제목만 다르고 체크박스 문법은 같으며, 「잘 동작한다」류를 거르는 규칙도 하나다. 재시도가 소진되면 `graphs/repair.py` 가 세 층 모두에 형식을 채워 넣고 `(확인 필요)` 를 붙인다.

⚠ 재설계·투입 그래프는 이 검사를 끈다 (`check_bodies=False`). 그 초안에는 이 규칙이 생기기 전에 저장된 주·태스크가 실려 있고, 그건 이번 변경이 고칠 대상이 아니다.
...
critic ──── 실패 ────► decompose (최대 3회 재시도)
  ...
  │   - 본문: 주 2 · 태스크 1 · 티켓 2개 이상의 확인 항목, 확인할 수 있는 말 (§2.2)
  ▼
emit
     DB 저장 + SSE 스트리밍 반환
```
