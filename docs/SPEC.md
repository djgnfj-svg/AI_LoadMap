...
| R3 | **1회 재설계 범위는 최대 마일스톤 1개** | 전체를 다시 그리면 사용자가 자기 계획이라고 느끼지 않음 |
...
**자를 수 있음**: 웹푸시(이메일만) / Export / clarify 다단계 질문(1회 고정으로 축소) / 로그인(데모 계정 고정)
...
자연어로 목표를 입력하면 **마일스톤 → 주차별 목표 → 실행 티켓**으로 분해되고, 동시에 시스템 아키텍처가 그려진다. 티켓을 완료하면 대응하는 아키텍처 노드가 채워진다. 지연·실패가 쌓이면 재점검일이 자동으로 잡히고, AI가 막힌 구간만 다시 설계한다.
...
```
프로젝트 (goal)
  └─ 마일스톤          예: "플레이 가능한 프로토타입"        3~6개
      └─ 주차별 목표    예: "2주차 - 캐릭터 이동/충돌"       마일스톤당 2~4개
...
3. 부분 재설계 (AI) — 최대 마일스톤 1개 범위 (R3)
   - 범위 축소 · 재분할 · 선행 학습 티켓 삽입 · 순서 교체
   - 영향받는 후속 마일스톤 일정 자동 이월
...
| `decompose` | 제약 객체 | 마일스톤/주차목표/티켓 트리 |
| `architect` | 제약 + 마일스톤 | 노드·엣지 그래프 |
| `link` | 티켓 + 노드 | 매핑 테이블 |
...
| 메인 (2분할) | 좌: 티켓 보드 / 우: React Flow 다이어그램 | **선택 시 양방향 하이라이트** |
...
| 재설계 결과가 부자연스러움 | 범위를 마일스톤 1개로 제한, diff 승인 필수 |
