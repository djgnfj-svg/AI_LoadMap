...
#### 도메인 — 게임 제작 하나 (2026-09-10 개정)

**만드는 것은 게임 하나로 좁힌다.** 09-09 에 「게임 · 소프트웨어 · 웹」 셋으로 정리했지만, 셋을 다 받으면 셋 다 어설프다. 접수까지 8일이고, 이 도구가 가장 잘 보이는 자리는 게임이다 — 시스템과 콘텐츠가 서로를 막아 세우는 게 눈에 보이고, 「넷코드가 안 돼서 보스 스테이지를 못 붙였다」가 그림 한 장으로 읽힌다.

축은 여전히 **무엇을 만드는가**다. 티켓을 끝내면 그림의 한 칸이 차오르는 게 이 도구의 전부라, 만들어지는 것이 있어야 성립한다. 점수나 습관에는 채울 칸이 없다.

| | 게임 제작 |
|---|---|
| 그림 이름 | 게임 구조도 |
| 노드 | 시스템 |
| `node_type` | system · stage · asset (+ `external`) |
| `layer` | play · rule · content · build |
| 티켓 본문 | 코드는 코딩 에이전트에, 에셋·레벨은 무엇을 몇 개 만들지 |
| 완료 조건 예시 | "친구 4명이 한 방에서 30분을 끊김 없이 돈다" |

⚠ **게임은 소프트웨어지만 낱말이 다르다.** 게임 로드맵에서 막히는 자리는 「백엔드」가 아니라 넷코드·보스 스테이지·사운드다. 그 낱말로 불러야 §2.3 의 알람이 진단으로 읽힌다 (R5).

**고르는 화면은 없다.** 새 로드맵은 전부 `game` 이고, 첫 화면은 곧장 「어떤 게임을 만드시나요?」를 한 줄로 묻는다 (자세한 것은 인터뷰 1번 청사진이 받는다, §3.3). `intake` 의 도메인 추정은 코드에 남아 있지만 화면이 늘 `game` 을 실어 보내므로 이기지 못한다 (`plan_graph.py` 의 `state.get("domain") or ...`).

`software` · `web` 프리셋은 **코드와 DB 제약에 남겨 둔다.** 지우는 게 아니라 안 내미는 것이다 — D10 에 넣은 자기 프로젝트(이 로드맵 자신, `software`)를 비롯해 이미 만든 로드맵이 제 낱말로 계속 읽혀야 하고, 확장은 화면 하나를 되돌리면 된다. 마이그레이션은 그래서 추가하지 않는다.
...
intake
  ...
  ▼
interview ──────────► [사용자 응답 대기]
  │  1라운드: 고정 질문 7개 (LLM 미개입). 라운드당 최대 7개.
  │           일 셋: 청사진 → 무엇이 들어가나(parts) → 지금 위치.
  │           사정 넷: 인원 → 실력 → 기한 → 주간 시간.
  │           intake 가 추측으로 채웠어도 묻는다 (추측은 되돌릴 자리가 없다).
...
| `architect` | 제약 + 주·태스크 + **문답 전문** | 노드·엣지 그래프 (`parts` 답이 노드가 된다) |
| `link` | 티켓 + 노드 | 매핑 테이블 |
| `critic` | 전체 | `{ok: bool, violations: []}` |
| `emit` | 검증 통과 결과 | DB 저장 |

#### 1라운드 질문 (2026-09-10 개정)

문구는 도메인 프리셋이 쥔다(`graphs/domains.py`). **field 와 순서는 도메인이 달라도 같다** — `apply_answers` 가 field 로 제약을 읽고 화면도 field 로 답을 맞춘다.

| # | | field | 게임 프리셋 문구 | 무엇이 되나 |
|---|---|---|---|---|
| 1 | 일 | `blueprint` | 당신이 만들고자 하는 게임의 청사진이 무엇인가요? | 완성 기준 초안 · decompose |
| 2 | 일 | `parts` | 이 게임에 무엇무엇이 들어가나요? | **architect 의 노드** · decompose |
| 3 | 일 | `starting_point` | 지금 어디까지 돼 있나요? | decompose (원문) |
| 4 | 사정 | `team_size` | 몇 명이 만드나요? | `constraints.team_size` |
| 5 | 사정 | `level` | 게임을 만들어 본 적 있나요? | `constraints.level` |
| 6 | 사정 | `deadline` | 언제까지 만드시나요? | `constraints.duration_weeks` |
| 7 | 사정 | `hours_per_week` | 한 주에 몇 시간 쓸 수 있나요? | `constraints.hours_per_week` |

**앞의 셋이 일을 쪼개고, 뒤의 넷이 사정을 잰다. 순서가 그 뜻이다.** 사람 사정은 계획의 **크기**만 정하고 무엇을 할지는 못 정한다. 「몇 명이 몇 시간」을 아무리 정확히 받아도 무엇이 들어가는지 모르면 남의 계획이 나온다.

**2번(`parts`)이 구조도를 사용자 것으로 만든다.** 여기 적은 것을 `architect` 가 그대로 노드로 만들고, 그 사람이 부른 이름을 label 로 쓴다. 전에는 문답 전문이 `decompose` 로만 가고 `architect` 는 목표 문장과 주·태스크만 봤다 — 그래서 게임을 만든다고 해도 노드가 「API 서버 · 데이터베이스」로 나왔다.

뒤의 넷은 `intake` 가 추측으로 채웠든 아니든 전부 묻는다 — 사람은 대개 이걸 안 세어 보고 시작하고, 틀린 추측은 되돌릴 자리가 없다.

1번은 **건너뛸 수 없다**(`interview.REQUIRED_FIELDS`). 나머지는 비우면 건너뛴 것으로 치고 2라운드가 필요하면 되묻는다.

⚠ **답은 사람 말로 온다.** 「저 혼자요」에는 숫자가 없고 「유니티로 3년 했어요」는 `advanced` 라는 낱말이 아니다. `apply_answers` 가 좁은 규칙으로 읽고, **못 읽으면 추론값을 그대로 둔다** — 애매한 답을 억지로 분류하느니 그게 낫다. 숫자는 **첫 것 하나만** 읽는다(「2~3명」이 23명이 되지 않게).

`done_when`(「무엇이 되면 쓸 수 있나요」)은 뺐다. 청사진 답과 겹쳤고, 완성 기준은 어차피 다음 화면에서 사용자가 직접 고쳐 확정한다.
...
### 3.7 티켓 투입 그래프 (Place Graph) — 2026-09-10 추가

```
propose ──► critic ──► diff ──► [사용자 승인 대기]
   ▲          │
   └── 실패 ──┘  (최대 3회)
```

**계획을 세워 주는 도구는 많다.** 계획대로 안 될 때 「이건 여기서 하고, 저건 저기서 이렇게」를 풀어 주는 것이 이 제품의 주장이고, 그 주장이 사는 자리가 이 그래프다.

만들다 보면 「아 이것도 해야 하네」가 생긴다. 그전까지 새 티켓이 들어올 문은 **재점검일 하나뿐**이었고, 그 문은 지연이 쌓여야 열렸다(§2.3). 그래서 떠오른 순간에 넣을 자리가 없었다.

| 노드 | AI | 하는 일 |
|---|---|---|
| `propose` | ○ | 계획 전체와 새 일 하나를 보고 **어느 태스크 · 무엇 다음 · 무엇 앞 · 어느 노드**인지 정한다 |
| `critic` | ✗ | 생성·재설계와 **같은 critic**. 120분 규칙, 주간 가용시간, 순환, 고아 노드 |
| `diff` | ✗ | 승인 단위로 내민다 |

재설계와 다른 점은 **범위**뿐이다. 재설계는 지연이 쌓인 노드에서 시작해 주 하나만 다시 그린다(R3). 여기서는 어디에 놓일지를 아직 모르므로 주를 미리 고를 수 없어 계획 전체를 본다.

내는 변경은 **재설계와 같은 타입**이다 — `add_ticket` 하나와 `add_dependency` 몇 개. 그래서 적용(`services/replan_apply.py`)·초안 미리보기(`replan.apply_to_draft`)·critic 이 이미 있는 것을 그대로 쓴다. **새로 낸 것은 문뿐이다.**

⚠ **AI 가 계획을 말없이 바꾸지 않는다.** 자리를 정하는 것까지가 AI 몫이고, 넣을지는 사용자가 항목별로 고른다 (청사진 확정 §3.3 · 재설계 diff §2.4 와 같은 결). `add_ticket` 을 거절하면 그것을 가리키던 선후관계도 조용히 건너뛴다.

| Method | Path | 설명 |
|---|---|---|
| POST | `/projects/{id}/tickets` | 새 일 하나 → 자리 제안 (**적용 안 함**) |
| POST | `/projects/{id}/tickets/{session_id}/apply` | 항목별 승인/거절 |

세션은 `replan_sessions` 에 `review_day_id = null` 로 남는다. 새 테이블을 만들지 않았다 — 승인 대기 중인 diff 라는 점에서 재설계 세션과 같은 것이다.
