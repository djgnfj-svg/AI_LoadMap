...
intake
  ...
  ▼
interview ──────────► [사용자 응답 대기]
  │  1라운드: 고정 질문 6개 (LLM 미개입). 라운드당 최대 6개.
  │           1번은 **청사진**. 2번부터는 전부 자기 사정을 짚어 보게 하는 질문이다 —
  │           인원 → 실력 → 지금 위치 → 기한 → 주간 시간.
...
|---|---|---|---|
| 1 | `blueprint` | 당신이 만들고자 하는 게임의 청사진이 무엇인가요? | 완성 기준 초안 · decompose 프롬프트 |
| 2 | `team_size` | 몇 명이 만드나요? | `constraints.team_size` |
| 3 | `level` | 게임을 만들어 본 적 있나요? | `constraints.level` |
| 4 | `starting_point` | 지금 어디까지 돼 있나요? | decompose 프롬프트 (원문) |
| 5 | `deadline` | 언제까지 만드시나요? | `constraints.duration_weeks` |
| 6 | `hours_per_week` | 한 주에 몇 시간 쓸 수 있나요? | `constraints.hours_per_week` |

**1번만 청사진이고 나머지는 자기 사정이다.** 계획의 크기를 정하는 것은 목표가 아니라 인원·실력·기간·주간 시간이고, 사람은 대개 이걸 안 세어 보고 시작한다. **세어 보게 하는 것 자체가 이 단계의 값이다.** 그래서 `intake` 가 추측으로 채웠든 아니든 전부 묻는다 — 틀린 추측은 되돌릴 자리가 없다.
