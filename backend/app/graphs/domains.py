...
class DomainPreset:
    ...
    # 갈리는 것은 묻는 말뿐이다.
    #
    # 순서도 도메인이 달라도 같다. **앞의 셋은 일 자체를 쪼개는 질문**이고
    # (청사진 → 무엇이 들어가나 → 지금 어디까지), 뒤의 넷이 자기 사정이다
    # (인원 · 실력 · 기간 · 주간 시간).
    #
    # 일이 먼저인 이유: 사람 사정은 계획의 **크기**만 정하고, 무엇을 할지는 못 정한다.
    # 「몇 명이 몇 시간」을 아무리 정확히 받아도 무엇이 들어가는지 모르면 남의 계획이
    # 나온다. 2번(parts)이 architect 의 노드가 되므로, 구조도는 사용자가 부른 이름으로
    # 그려진다 — LLM 이 낱말을 지어내다 도메인을 벗어나던 자리가 여기다.
    #
    # 뒤의 넷은 추론에 맡기지 않고 전부 묻는다 — 틀린 추론은 되돌릴 자리가 없다.
