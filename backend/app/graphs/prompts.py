...
- domain: **무엇을 만드는가**로 고른다. 셋뿐이다.
  * "game": 게임을 만든다 (장르·플랫폼 무관, 게임잼 포함)
  * "web": 브라우저로 여는 것을 만든다 (사이트, 웹 서비스, 웹앱)
  * "software": 그 밖의 프로그램 (데스크톱 앱, 모바일 앱, CLI, 봇, 라이브러리)
  ⚠ 보통은 사용자가 첫 화면에서 이미 골라 넘긴다. 그 값이 있으면 그대로 쓴다.
