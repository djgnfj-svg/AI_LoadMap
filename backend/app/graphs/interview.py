...
# 5 였던 것을 7 로 올렸다 — 다섯 칸이 한 화면에 쏟아지던 때의 숫자였고, 지금은
# 한 번에 하나씩 내민다. 1라운드 고정 질문(일 셋 + 사정 넷)이 7개다.
...
#
# **앞의 셋이 일 자체를 쪼갠다**: 청사진 → 무엇이 들어가나(parts) → 지금 어디까지.
# 1번은 「목표가 뭐예요」가 아니라 「무엇을 만드나요」로 묻는다 — 앞의 질문에는
# 하고 싶은 것을 적고, 뒤의 질문에는 만들 것을 적기 때문이다.
#
# 뒤의 넷이 자기 사정이다: 인원 · 실력 · 기간 · 주간 시간. 이쪽은 계획의 **크기**만
# 정하고 무엇을 할지는 못 정한다. 그래서 일이 먼저다. 다만 사람은 대개 이걸 안
# 세어 보고 시작하므로, intake 가 추측했든 아니든 **전부 묻는다** — 틀린 추측은
# 되돌릴 자리가 없다.
...
def first_round(domain: str | None = None) -> list[ClarifyQuestion]:
    """1라운드 질문. 도메인 프리셋이 통째로 쥔다 (일 셋 + 자기 사정 넷)."""
